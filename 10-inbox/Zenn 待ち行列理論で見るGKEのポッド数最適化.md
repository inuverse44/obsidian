...
def calculate_average_queue_time(arrival_rate: float, service_rate: float, servers: int) -> float | None:
    """
    Calculates the average waiting time in the queue (Wq) for an M/M/c queue.

    This model assumes Poisson arrivals, exponential service times, and 'c' parallel servers.
    Returns None if the system is unstable (traffic intensity >= 1).

    Args:
        arrival_rate (λ): Average number of arrivals per unit of time.
        service_rate (μ): Average number of services per unit of time per server.
        servers (c): Number of parallel servers (e.g., pods).

    Returns:
        The average waiting time (Wq), or None for an unstable system.
    """
    # System is unstable if arrival rate is greater than or equal to total service capacity.
    if arrival_rate >= servers * service_rate:
        return None

    # r: Offered traffic (in Erlangs)
    # rho: Traffic intensity or server utilization
    r = arrival_rate / service_rate
    rho = r / servers

    # P0: Probability that all servers are idle.
    # This is calculated using the Erlang C formula's components.
    ...
    # Wq: Average waiting time in the queue (Little's Law).
    wq = lq / arrival_rate

    return wq

def find_optimal_pods(arrival_rate: float, service_rate: float, slo_p95_ms: float, max_pods: int = 100) -> tuple[int, float] | tuple[None, None]:
    """
    Finds the minimum number of pods (servers) to meet a 95th percentile response time SLO.

    Args:
        arrival_rate (λ): Total requests per second.
        service_rate (μ): Requests per second a single pod can handle.
        slo_p95_ms: The Service Level Objective for the 95th percentile response time in milliseconds.
        max_pods: The maximum number of pods to consider.

    Returns:
        A tuple containing the optimal number of pods and the resulting P95 response time,
        or (None, None) if the SLO cannot be met.
    """
    # Start checking from the minimum number of pods required to handle the load.
    min_pods = math.floor(arrival_rate / service_rate) + 1
    service_time_sec = 1.0 / service_rate
    slo_p95_sec = slo_p95_ms / 1000.0

    for num_pods in range(min_pods, max_pods + 1):
        wq_sec = calculate_average_queue_time(arrival_rate, service_rate, num_pods)

        if wq_sec is not None:
            # P95 Wait Time ≈ 3 * Average Wait Time (a common heuristic for M/M/c queues).
            # P95 Response Time = P95 Wait Time + Service Time.
            p95_response_time_sec = (wq_sec * 3) + service_time_sec

            if p95_response_time_sec <= slo_p95_sec:
                return num_pods, p95_response_time_sec * 1000  # Return result in ms
...
if __name__ == "__main__":
    ...
    # μ: Service rate per pod (requests/sec)
    # SLO: Target for 95th percentile response time (ms)
    ARRIVAL_RATE_RPS = 100.0
    SERVICE_RATE_RPS_PER_POD = 20.0
    SLO_P95_RESPONSE_TIME_MS = 200.0

    # --- Print System Info ---
    print("--- System Parameters ---")
    print(f"Arrival Rate (λ): {ARRIVAL_RATE_RPS} req/s")
    print(f"Service Rate per Pod (μ): {SERVICE_RATE_RPS_PER_POD} req/s")
    print(f"SLO (P95 Response Time): < {SLO_P95_RESPONSE_TIME_MS} ms")
    print("-" * 25)

    # --- Find Solution ---
    optimal_pods, resulting_p95 = find_optimal_pods(
        ARRIVAL_RATE_RPS,
        SERVICE_RATE_RPS_PER_POD,
        SLO_P95_RESPONSE_TIME_MS
    )

    # --- Display Result ---
    if optimal_pods is not None:
        print("✅ SLO Met!")
        print(f"Minimum required pods (c): {optimal_pods}")
        print(f"Resulting P95 Response Time: {resulting_p95:.2f} ms")
    else:
        print("❌ SLO could not be met within a reasonable number of pods.")
```
